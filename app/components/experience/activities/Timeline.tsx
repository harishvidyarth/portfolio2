...
import { ACTIVITIES_TIMELINE } from "@constants";
  ...
  return (
    <group position={point.point} scale={isMobile ? 0.35 : 0.6}>
      <Box args={[0.2, 0.2, 0.2]} position={[0, 0, -0.1]} scale={[1 - diff, 1 - diff, 1 - diff]}>
        <meshBasicMaterial color="#b83a24" wireframe />
        <Edges color="#b83a24" lineWidth={1.5} />
      </Box>
      <group>
...
const Timeline = ({ progress }: { progress: number }) => {
  ...
  return (
    <group position={[0, -0.1, -0.1]}>
      <Line points={visibleCurvePoints} color="#b83a24" lineWidth={3} />
